...
- (void)initViews {
    ...
    [_contentView1 addSubview:_view1];
    [_contentView1 addSubview:_view2];
    
    CGFloat padding = 10;
    [_view1 mas_makeConstraints:^(MASConstraintMaker *make) {
        //
        make.top.equalTo(_stepper1.mas_bottom).and.offset(padding);
        make.left.equalTo(_contentView1.mas_left).and.offset(padding);
        make.right.equalTo(_view2.mas_left).and.offset(-padding);
        make.height.mas_equalTo(@80);
        make.width.mas_equalTo(_view2);
    }];
    
    [_view2 mas_makeConstraints:^(MASConstraintMaker *make) {
        //
        make.top.mas_equalTo(_view1);
        make.left.equalTo(_view1.mas_right).and.offset(padding);
        make.right.equalTo(_contentView1.mas_right).and.offset(-padding);
        make.height.mas_equalTo(@80);
        make.width.mas_equalTo(_view1);
    }];
